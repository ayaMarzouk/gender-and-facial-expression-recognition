...
def HOFKNN(test_dir):
    ...
    GenderClf = None
    HOFexpressionClf = None
    GenderClf = None
    HOFexpressionClf = None
    HOG_features = np.load('HOG_features.npy')
    HOF_features = np.load('X_train.npy')
    Gender_train = np.load('Gender_train.npy')
    expression_train = np.load('y_train.npy')
    

    
    print (HOF_features.shape)
    npos = np.where(np.isnan(HOF_features))
    for i,j in zip(npos[0],npos[1]): HOF_features[i,j] = 0
    GenderClf = KNeighborsClassifier (n_neighbors = 10)
    GenderClf.fit(HOG_features,Gender_train)

    HOFexpressionClf = KNeighborsClassifier (n_neighbors = 10)
    HOFexpressionClf.fit(HOF_features,expression_train)
    ...
    while 1:
        frameId = cap.get(1)
        ret, frame2 = cap.read()
        if not ret or NInterval==7: break
